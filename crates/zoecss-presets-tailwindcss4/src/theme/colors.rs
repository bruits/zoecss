...
mod tests {
    ...
    #[test]
    fn uses_oklch_values() {
        let mut theme = Theme::default();
        register(&mut theme);
        assert_eq!(
            theme.get("color", "red-500"),
            Some("oklch(63.7% 0.237 25.331)")
        );
        assert_eq!(
            theme.get("color", "blue-500"),
            Some("oklch(62.3% 0.214 259.815)")
        );
        assert_eq!(theme.get("color", "black"), Some("#000"));
        assert_eq!(theme.get("color", "white"), Some("#fff"));
    }

    #[test]
    fn all_palettes_present() {
        let mut theme = Theme::default();
        register(&mut theme);
        for palette in [
            ...
            for shade in [
                ...
                let key = format!("{palette}-{shade}");
                assert!(theme.get("color", &key).is_some(), "missing color: {key}");
            }
        }
    }
}
